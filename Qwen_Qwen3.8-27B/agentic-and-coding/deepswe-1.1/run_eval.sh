#!/usr/bin/env bash
# DeepSWE 1.1 via the Harbor hub on Qwen/Qwen3.8-27B against an already-running vLLM server.
#
...
# --max-sandboxes equals --max-connections: Inspect holds one container per in-flight sample. 24 in flight is what
# the server's KV cache holds (4.0M tokens at TP=8) for this task, whose agent histories average about 120k tokens
# per request (56, as for SWE-bench Pro, saturated the cache: preemptions, 1-7% prefix-cache hits, every turn re-prefilled).
# Retune a running eval with `inspect ctl config --key vllm/<model> N --max-samples N --key docker N`.
...
inspect eval inspect_harbor/harbor \
  ...
  --max-connections "${MAX_CONNECTIONS:-24}" \
  --max-sandboxes "${MAX_SANDBOXES:-24}" \
